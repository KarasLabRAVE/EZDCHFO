...
pipeline_xls <- readxl::read_xlsx("/Volumes/bigbrain/ACL/RAVE_Projects/PipelineScripts/EpileptogenicIndex/FragilityEEGDataset_pipeline_update_050924.xlsx")
...
pathres="/Volumes/bigbrain/ACL/RAVE_Projects/PipelineScripts/DCHFO/Results/"
scaling=1000000 

time_window_ictal <- c(-10, 20)
time_window <- c(-30, 30)
reference_name <- "car"
...
for(i in pts){
  ...
  for(j in ictal_runs){
    ...
    condition=paste('30s before sz',j,sep="")
    print(condition)
    ...
    tsBaseline[1:ntb,1:nel]=inputtimeseries[1:ntb,1:nel]
    
    meanbs<-rowMeans(t(tsBaseline))
    ...
    ntt <- ncol(electrodes_by_time)
    nel <- length(electrodes)
    fs <- repository$sample_rate
    dt <- as.numeric(1/fs)
    timeserieictal <- t(electrodes_by_time)
    
    ts <- 1:nt
    ts <- ts*dt
    inputtimeseries<-timeserieictal[its:ite,1:nel]
    ...
    for(ie in 1:nel){
      ...
      sigedc=filter(but,sige)-meanbs[ie]
      lowpassts[1:nt,ie]<-sigedc
      
    }
    
    
    tsEpochv<-data.frame(lowpassts)
    colnames(tsEpochv)<-channelDefs$name
