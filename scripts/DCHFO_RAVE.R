...
# Parameters for multitaper HFO analysis
frequency_range = c(80, 250)  # HFO
...
# which patients to run from the multipattient data sheet data
pts <- dipsaus::parse_svec("1:16")

# listing patient, seizure, etc.
pipeline_xls <- readxl::read_xlsx("/Volumes/bigbrain/ACL/RAVE_Projects/PipelineScripts/EpileptogenicIndex/FragilityEEGDataset_pipeline_update_050224.xlsx")
...
for(i in pts){
  ...
  for(j in ictal_runs){
    ...
    sige   <- vector(mode="numeric", length=nt)

    lowpassts=inputtimeseries
    ...
    lt2=2*fs
     ...
     lengthdce<-vector(mode="numeric", length=nel)
     maxdce<-vector(mode="numeric", length=nel)
     startdce<-vector(mode="numeric", length=nel)
  ...
  for(ie in 1:nel){
    ...
    specthfo = results[[1]]
    hfoie=colMeans(specthfo)
    heatmaphfo[ie,1:nwt]=hfoie[1:nwt]

  }


  stimes = results[[2]]+time_window[1]
  sfreq=results[[3]]


heatmaphfo<-heatmaphfo[elecsozsozc,]


hfomap_data <- expand.grid(Time = stimes, Electrode = elecnum)
hfomap_data$Value <- c(t(heatmaphfo))

titlepng=paste(subject_code,'Seizure',as.character(j),sep=" ")

ggplot(hfomap_data, aes(x = Time, y = Electrode, fill = Value)) +
  geom_tile() +
  ggtitle(titlepng)+
...
resfile=paste(pathres,'HFO_',subject_code,'_',condition,'.png')
ggsave(resfile)

        maxhfopow=max(heatmaphfo)
        threshold=maxhfopow*0.1


        testhfo<-vector(mode="numeric", length=nel)
        lengthhfo<-vector(mode="numeric", length=nel)
        maxhfo<-vector(mode="numeric", length=nel)
        starthfo<-vector(mode="numeric", length=nel)
        ...
        for(ie in 1:nel){
          ...
          hfolengthp<-max(resulthfo$length)*0.2

          if(hfolengthp>lthfo){
            testhfo[ie]=1
            lengthhfo[ie]=hfolengthp
            maxhfo[ie]=max(sige)
            starthfo[ie]=seq[1]*0.2
          }

        }

        testdce=testdce[elecsozsozc]
        lengthdce=lengthdce[elecsozsozc]/fs
        startdce=startdce[elecsozsozc]/fs
        maxdce=maxdce[elecsozsozc]

        dfres=data.frame(elecnum,testdce,lengthdce,startdce,maxdce,testhfo,lengthhfo,starthfo,maxhfo)


        resfile=paste(pathres,'dchfo_',subject_code,'_',condition,'.csv')
